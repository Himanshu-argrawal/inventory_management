...
var Invoice = {
  ...
  addItem: function() {
      ...
      d = "price"+i;
      e = "quantity"+i;
    var html = '<tr id="item-2"><td>'+k+'</td><td><input type="number" class="price" name="'+d+'" step="0.01" value="0.00"></td><td><input type="number" class="quantity" name="'+e+'" step="1" value="1"></td><td><select name="tax" id="" class="tax_rate"><option value="">None</option><option value="21">IVA 21%</option><option value="14">IVA 14%</option></select></td><td class="total-price">€ 0,00</td><td><a href="#" class="delete">x</a></td></tr>';
    $('#items').append(html);
    for(p=0 ; p < opt_values.length ; p++)
    {
        var dd = opt_values[p];
        $('#item-2 select').append('<option value ="'+dd+'">'+dd+'</option>');
    }
      i = i+1;
